...
function getIndex(context, path, index) {
...
var uikit = module.exports = y.toAPI('uikit', {
		...
		return this
			.toMethods({
				patchItem: function(e) {
					if (!applyRemote)
						return;
					// console.log('patchItem : ', opt.protocol, this.data.id, e.value, e.context.path + '.' + e.path);
				...
				deleteItem: function(path, index) {
					...
					if (!applyRemote)
						return;
					y.c3po.remote(opt.protocol, 'deleteitem', { id: this.data.id, path: opt.path, index: index })
					...
					.each(opt.path,
						...
						function(eacher, contextEacher, context, node, container) {
							eacher.on('pushItem', function(itemContainer) {
								if (!applyRemote)
									return;
								y.c3po.remote(opt.protocol, 'pushitem', { id: context.data.id, item: itemContainer.context.output(), path: opt.path  })
	...
	posterH1: function() {
		return this
			.client(y().posterItemMenu( /* ?menuTempl */ ))
			.h(1,
				y().contentEditable({
					value: '{{ content }}',
