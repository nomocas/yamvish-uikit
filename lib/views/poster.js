// --buddyfile: ../../scss/views/_test.scss

var y = require('yamvish'),
	embed = require('embed-video'),
	fitVids = require('nomocas-webutils/lib/fitvids');

y.utils.validUri = /^(https?:\/\/(?:www\.|(?!www))[^\s\.]+\.[^\s]{2,}|www\.[^\s]+\.[^\s]{2,})/;
...
y.toAPI('poster', {
	...
	poster: function(opt /*{ path:'my.items', editModePath, menu, allowAddItem }*/ ) {
		...
		return this.dom(function(context, node) {
			...
			.div(
				y().cl('poster-items')
				.if(opt.swapable, y().swapableParent(opt.path, '{{ ' + opt.editModePath + ' }}'))
				.each(opt.path, y().use('poster:item', opt.swapable, opt.editModePath, menuPath, opt.allowAddItem))
			)
			.if(opt.allowAddItem, y().use('poster:endMenu', opt.editModePath, menu))
	},
			...
			y().toMethods({
				execute: function(e, data) {
					if (typeof this.data.insertToIndex !== 'undefined')
						this.parent.insertItem(path, { data: data, index: this.data.insertToIndex });
					else
	...
	endMenu: function(editModePath, menu) {
		return this.mountIf('{{ ' + editModePath + ' }}',
			y().div(
				...
				.button(
					...
					.click(function(e) {
						e.stopPropagation();
						this.toggle('$ui.showButtonsPanel');
					})
				)
				.mountIf('{{ $ui.showButtonsPanel }}', menu)
			)
		);
	},

	//__________________________________________________ POSTER ITEM WRAPPER
	item: function(swapable, editModePath, menuPath, allowAddItem) {
		editModePath = '$parent.' + editModePath;
		var editFlag = y.interpolable('{{ ' + editModePath + ' }}');
		return this.div(
			y().cl('poster-item')
			.cl('edited-mode', editFlag)
			.if(swapable, y().swapableItem())
	...
	defaultMenu: function(templ) {
		return this.toMethods({
				addH1: function(e) {
					this.call('execute', e, { layout: 'poster:h1', content: 'First Level Title' });
				},
				addH2: function(e) {
					this.call('execute', e, { layout: 'poster:h2', content: 'Second Level Title' });
				},
				addHeadline: function(e) {
					this.call('execute', e, { layout: 'poster:headline', content: 'A cool headline...' });
				},
				addText: function(e) {
					this.call('execute', e, { layout: 'poster:text', content: 'Some text...' });
				},
				addImage: function(e) {
					this.call('execute', e, { layout: 'poster:image', uri: '/statics/img/avatar.png' });
				},
				addVideo: function(e) {
					this.call('execute', e, { layout: 'poster:video', uri: '' });
				}
			})
			...
			.div(
							...
							var update = function(value) {
								if (value) {
									...
									if (!output)
										...
										y.utils.insertHTML(output, node);
				...
				.mountIf('{{ !uri || $error.badURI }}',
					y().input('text', '',
						...
						.visible(editFlag)
						.on('input', function(e) {
							if (e.target.value && !y.utils.validUri.test(e.target.value))
